           ...
           if (this.readyState == 4 && this.status == 200) {
               ...
               if(obResultado[0]["retorno"]=="TRUE"){ 
                  ...
                  l_Tamaxo=12/l_Encabezados.length;
                   ...
                   for(i=0;i<l_Encabezados.length;i++){
                       ...
                       if(l_Llave!=l_Encabezados[i]){
                           l_Linea=l_Linea + "<div class='col-md-1 d-flex justify-content-left align-items-center w-3' style='height: 40px;text-align:center; font-size:10px; font-family:Arial black, Gadget, sans-serif;vertical-aling:middle;display:block; cursor:pointer;background-color:#F4F5F6;'>";
                   ...
                   */
                   ...
                   l_Linea=l_Linea + "<div class='col-md-1 d-flex justify-content-left align-items-center w-3' style='height: 40px;text-align:center; font-size:10px; font-family:Arial black, Gadget, sans-serif;vertical-aling:middle;display:block; cursor:pointer;background-color:#F4F5F6;'>";
                   l_Linea=l_Linea + "<label id='campo_" + i + "'  onclick=\"fn_Listado_Ordenar_Clic('" + l_Encabezados[3] + "')\">";
                   l_Linea=l_Linea +  l_Encabezados[3];
                   l_Linea=l_Linea + "<img src='../iconos/updown.png' style='width: 10px; height: 10px; margin-left:2px;'>";
                   l_Linea=l_Linea + "</label>";
                   l_Linea=l_Linea + "</div>";

                   l_Linea=l_Linea + "<div class='col-md-3 d-flex justify-content-left align-items-center w-3' style='height: 40px;text-align:center; font-size:10px; font-family:Arial black, Gadget, sans-serif;vertical-aling:middle;display:block; cursor:pointer;background-color:#F4F5F6;'>";
                   ...
                   for(i=0;i<contador;i++){     
                       ...
                       */
                       ...
                       l_Linea=l_Linea + "<div class='col-md-1 justify-content-start align-items-left w-3 d-inline-block ' >";
                       l_Linea=l_Linea + "<label style='cursor:pointer;' >" +   obResultado[0]["Folio"] + "</label>";
                       l_Linea=l_Linea + "</div>"; 

                       l_Linea=l_Linea + "<div class='col-md-2 justify-content-start align-items-left w-3 d-inline-block ' >";
                       l_Linea=l_Linea + "<label style='cursor:pointer;' >" +   obResultado[0]["Fecha"] + "</label>";
                       l_Linea=l_Linea + "</div>"; 

                       l_Linea=l_Linea + "<div class='col-md-2 justify-content-start align-items-left w-3 d-inline-block ' >";
                       l_Linea=l_Linea + "<label style='cursor:pointer;' >" +   obResultado[0]["Almacen"] + "</label>";
                       l_Linea=l_Linea + "</div>"; 

                       l_Linea=l_Linea + "<div class='col-md-1 justify-content-start align-items-left w-3 d-inline-block ' >";
                       l_Linea=l_Linea + "<label style='cursor:pointer;' >" +   obResultado[0]["Rack"] + "</label>";
                       l_Linea=l_Linea + "</div>"; 

                       l_Linea=l_Linea + "<div class='col-md-3 justify-content-start align-items-left w-3 d-inline-block ' >";
                       l_Linea=l_Linea + "<label style='cursor:pointer;' >" +   obResultado[0]["Nombre"] + "</label>";
                       l_Linea=l_Linea + "</div>";

                       l_Linea=l_Linea + "<div class='col-md-1 justify-content-start align-items-left w-3 d-inline-block ' >";
                       l_Linea=l_Linea + "<label style='cursor:pointer;' >" +   obResultado[0]["Estatus"] + "</label>";
                       l_Linea=l_Linea + "</div>";
                       ...
                       for (var campo in obResultado[i]) {
                           if(campo!="retorno" && campo!="msg" && campo!="llave" && campo!="especiales" && campo!="combo" && campo!="encabezados" ){
                               if(l_Llave==campo){
                                   l_Linea=l_Linea + "<div class='col-md justify-content-start align-items-left w-3 d-inline-block ' >";
                                   l_Linea=l_Linea  + "<label style='display:inline-flex' >";
                                   l_Linea=l_Linea  + "<a class='nav-link' href='" + MODULO + "_editar.php?id=" + obResultado[i][campo] +"&accion=Detalles' style='color: #53BEFE;cursor:pointer; margin-left: -15px; margin-right: -10px;font-weight:bold; '>Detalle </a>"
                                   //l_Linea=l_Linea  + "</label>";
                             

                                   if(l_Estatus=="ABIERTO"){
                                       //l_Linea=l_Linea  + "<label style='display:inline-flex' >";
                                       l_Linea=l_Linea  + "<a class='nav-link' href='" + MODULO + "_editar.php?id=" + obResultado[i][campo] +"&accion=Cancelar' style='color: #53BEFE;cursor:pointer;margin-right: -10px; font-weight:bold;' >Cancelar</a>";
                                       l_Linea=l_Linea  + "<a class='nav-link' href='#' style='color: #53BEFE;cursor:pointer;margin-right: -10px;font-weight:bold;' ></a>";                                                                                           
                                       //l_Linea=l_Linea  + "</label>";
                                   } else {
                                        if(l_Estatus=="PROCESO"){
                                            //l_Linea=l_Linea  + "<label style='display:inline-flex' >";
                                            l_Linea=l_Linea  + "<a class='nav-link' href='" + MODULO + "_editar.php?id=" + obResultado[i][campo] +"&accion=Ajustar' style='color: #53BEFE;cursor:pointer;margin-right: -10px;font-weight:bold;' >Ajustar</a>";                                                                                           
                                            l_Linea=l_Linea  + "<a class='nav-link' href='" + MODULO + "_editar.php?id=" + obResultado[i][campo] +"&accion=Cancelar' style='color: #53BEFE;cursor:pointer;margin-right: -10px;font-weight:bold;' >Cancelar</a>";
                                            //l_Linea=l_Linea  + "</label>";
                                        }   else {
...
function Inventario_Combo(datos){ 
    ...
    if(datos.length>0){
        ...
        var xmlhttp = new XMLHttpRequest();
        var url = UBICACION_CONTROL + "/inventarioxubicacion_consultar_todos.ctrl.php";
        ...
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                document.getElementById("cb_nIDIxU").innerHTML=l_Registros;  
                // ----------------------------------------------------------------             
