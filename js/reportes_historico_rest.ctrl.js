...
function Listado_Detalles(datos){        
    if(datos.length>0){
        ...
        var url = UBICACION_CONTROL + "/reporte_historico_todos.ctrl.php";
           ...
           if (this.readyState == 4 && this.status == 200) {
            ...
            for(i=0;i<contador;i++){     
                ...
                if(obResultado[i]["retorno"]=="TRUE"){ 
                    fn_Ver_Detalles(obResultado[i]["nIDProducto"], obResultado[i]["Codigo_IZeta"], obResultado[i]["Producto"], obResultado[i]["UnidadDeMedida"], obResultado[i]["Cantidad"], obResultado[i]["Almacen"],obResultado[i]["Tipo"] );
                } else {
...
function fn_Ver_Detalles(l_nIDProducto, l_Codigo, l_Producto, l_UnidadDeMedida,l_Cantidad, l_Almacen, l_Tipo ){
     
    l_Codigo=l_Codigo.trim();
    l_Producto=l_Producto.trim();
    l_UnidadDeMedida=l_UnidadDeMedida.trim();
    
    listadodeproductos.push( {"nidproducto":l_nIDProducto,"codigo": l_Codigo, "producto":l_Producto, "unidaddemedida":l_UnidadDeMedida, "cantidad":l_Cantidad, "almacen":l_Almacen, "tipo":l_Tipo} );     
    ...
    l_Linea=l_Linea + "<label style='cursor:pointer;' > Almacen</label>";
    l_Linea=l_Linea + "</div>";

    l_Linea=l_Linea + "<div class='col d-flex justify-content-left align-items-center' style='text-align:center; font-size:10px; font-family:Arial black, Gadget, sans-serif;vertical-aling:middle;display:block; cursor:pointer;background-color:#F4F5F5;'>";
    l_Linea=l_Linea + "<label style='cursor:pointer;' > Tipo</label>";
    l_Linea=l_Linea + "</div>";
    ...
    for(i=0;i<listadodeproductos.length;i++){
        ...
        l_Linea=l_Linea + "<div class='col d-flex justify-content-start align-items-left' style='text-align:center; font-size:8px; vertical-aling:middle;display:block; cursor:pointer;'>";
        l_Linea=l_Linea + "<label style='cursor:pointer;' >" +   listadodeproductos[i]["almacen"] + "</label>";
        l_Linea=l_Linea + "</div>";  

        l_Linea=l_Linea + "<div class='col d-flex justify-content-start align-items-left' style='text-align:center; font-size:8px; vertical-aling:middle;display:block; cursor:pointer;'>";
        l_Linea=l_Linea + "<label style='cursor:pointer;' >" +   listadodeproductos[i]["tipo"] + "</label>";
        l_Linea=l_Linea + "</div>";  
