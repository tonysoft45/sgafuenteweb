...
function fn_Incrementar_Clic(){

    var nIDIxA = document.getElementById("txt_nIDIxA").value;
    var nIDUsuario = document.getElementById("txt_nIDUsuario").value;
    var nIDProducto = document.getElementById("cb_nIDCat_Producto").value;
    var Cantidad = document.getElementById("txt_Cantidad").value;
    var Descripcion = document.getElementById("txt_Descripcion").value;
    var l_Funcion = "INCREMENTAR";

    Descripcion=Descripcion.trim();

    // Verificar 
    if(nIDIxA.length<=0){        
        document.getElementById("lbl_mensaje_falla").innerHTML="NO TIENE ID DEL INVENTARIO X ARTICULO";          
        $("#modal_falla").modal("show");
        return;
    }

    if(nIDProducto.length<=0){        
        document.getElementById("lbl_mensaje_falla").innerHTML="SELECCIONA EL PRODUCTO";          
        $("#modal_falla").modal("show");
        return;
    }

    if(Cantidad.length<=0){        
        document.getElementById("lbl_mensaje_falla").innerHTML="TIENE QUE CAPTURAR LA CANTIDAD";          
        $("#modal_falla").modal("show");
        return;
    }

    if(Descripcion.length<=0){        
        document.getElementById("lbl_mensaje_falla").innerHTML="CAPTURE LA DESCRIPCION";          
        $("#modal_falla").modal("show");
        return;
    }
    ...
    // Validaciones
    if(nIDProducto<=0){         
        document.getElementById("lbl_mensaje_falla").innerHTML="DEBE DE SELECCIONAR EL PRODUCTO";          
        $("#modal_falla").modal("show");
        return;
    }

    if(!validarSiNumero(Cantidad)){
        console.log("LA CANTIDAD DEBE DE SER NUMERICA");
    ...
    var arreglo=new Array();
    arreglo.push( {"nidixa":nIDIxA, "nidusuario":nIDUsuario, "nidproducto":nIDProducto, "cantidad":Cantidad, "descripcion":Descripcion, "funcion":l_Funcion } );     
