...
function fn_Inventario_Clic(l_Consulta,l_Seleccion,l_Accion){     
    // Verificaciones
    if(l_Consulta.length<=0){
        console.log("ERROR NO TIENE CONSULTA");
        //return;
    ...
    if(l_Consulta.length>0){
        l_Consulta=l_Consulta + "and bEstado=0 and (Estatus='ABIERTO' or Estatus='PROCESO')";
    } else {
        l_Consulta="bEstado=0 and (Estatus='ABIERTO' or Estatus='PROCESO')";
    }
    ...
    if(arreglo.length>0){       
        // Enviar para procesdar
        resultado=Inventario_Combo(arreglo); 
    } else {
        console.log("ERROR NO SE CARGO LA INFORMACIÓN");
    }
}
